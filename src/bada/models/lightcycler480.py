...
class LightCycler480Raw(pa.DataFrameModel):
    ...
    _well: Series[float] = pa.Field(
        regex=r"^[A-P](?:[1-9]|1[0-9]|2[0-4]): Sample \d+$",  # ty:ignore[invalid-argument-type]
        alias=True,
    )

    # allow X.n columns where n is 1-1536
    _x_n: Series[float] = pa.Field(
        regex=r"^X\.(?:[1-9]|[1-9][0-9]|[1-9][0-9][0-9]|1[0-4][0-9][0-9]|15[0-2][0-9]|153[0-6])$",  # ty:ignore[invalid-argument-type]
        alias=True,
    )
